    ...
    return (
        <div>
            <table class="table">
                {
                    keys ? keys.map(data => {
                        return <NodeLine hash={msgs[data]} />
                    }) : "Nothing Data"
                }
            </table>
        </div>
...
export const BbsNodeTop = (props) => {
    let address = props.match.params.id;
    return (
        [
            <BbsNode address={address} />,
            <NodeCreate address={address} />,
        ]
    );
