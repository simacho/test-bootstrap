...
const VertexCreate = (props) => {
    ...
    return (
        <div>
            <Form onSubmit={(e)=>vtx.creae(e,address,name)} >
    ...
    const create = (ev:InputEvent , address , name) => { 
        try {
            ...
            firebaseDb.ref(fullname).once("value",(snapshot) => {
                if ( snapshot.val() ) throw new Error("vertex name error")
            })
...
export {  VertexContext , VertexProvider }
